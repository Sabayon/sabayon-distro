...
src_unpack () {
	...
	# Fix Ethernet Link detection
	epatch ${FILESDIR}/${P}-fix-ethernet-link-detection.patch

	# Sometimes device gets downed during wpa_supplicant association
	epatch ${FILESDIR}/${P}-stage3-device-always-up.patch

	# some drivers are dumb
	epatch ${FILESDIR}/${P}-sleep-one-second.patch
