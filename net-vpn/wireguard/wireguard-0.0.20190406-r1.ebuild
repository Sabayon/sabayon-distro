...
inherit linux-mod bash-completion-r1 linux-info sabayon-kernel-utils
...
src_prepare() {
	if kernel_is -gt 4 19 10 ; then
		# Revert patch 65e1f2838401bf033f3c8a8cdd718589574d9d2d
		# because package use linux-headers package that is
		# not the same version of /usr/src/linux directory.
		epatch "${FILESDIR}"/${PN}-revert_65e1f2838401bf033f3c8a8cdd718589574d9d2d.patch
	fi
	eapply_user
}
...
src_compile() {
	...
	use tools && emake RUNSTATEDIR="${EPREFIX}/run" \
		-C src/tools CC="$(tc-getCC)" LD="$(tc-getLD)"
}
