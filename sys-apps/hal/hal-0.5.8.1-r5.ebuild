...
src_unpack() {
	...
	epatch ${FILESDIR}/${P}-sabayonlinux-ntfs-fuse.default.patch

	#### From FC7 src.rpm

	# do not crash on cdrom drives - aka r/o storage
	epatch ${FILESDIR}/${P}-dont-crash-on-cdrom-drives.patch

	# fix partutil
	epatch ${FILESDIR}/${P}-partutil-fix.patch

	# remove link only
	epatch ${FILESDIR}/${P}-remove-link-not-remove.patch

	# wrong stat
	epatch ${FILESDIR}/${P}-stat-devicefile-not-mountpoint.patch

	# align !
	epatch ${FILESDIR}/${PN}-alignment.patch

	# Add TabletPC magic
	epatch ${FILESDIR}/${P}-add-tabletPC-support.patch

	# add DeviceScape Stack support (for wireless-dev git tree)
	epatch ${FILESDIR}/${P}-new-dscape-stack-support.patch

	### From ARCH Linux
	
	# check for error in case of floppy or cdrom drives
	epatch ${FILESDIR}/${P}-check-for-error.patch
