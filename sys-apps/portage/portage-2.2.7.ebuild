...
pkg_postinst() {
	...
	if ${REPOS_CONF_UPGRADE} ; then
		...
		# Entropy <= 223 set PORTDIR to a fake directory, if we
		# detect that, we assume that we're running inside Entropy.
		local portdir="${PORTDIR}"
		if [[ ${PORTDIR} == /var/tmp/entropy/* ]]; then
			einfo "Setting PORTDIR to /usr/portage, running within Entropy"
			portdir=/usr/portage
		fi
