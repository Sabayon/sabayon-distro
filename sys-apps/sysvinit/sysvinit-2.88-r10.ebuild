...
src_prepare() {
	...
	# /sbin/init renamed to /sbin/sysvinit
	sed -i "/^#define INIT/ s:/sbin/init:/sbin/sysvinit:" \
...
src_install() {
	...
	# disable eselect-init support, we only support systemd now
	mv "${D}"/sbin/init "${D}"/sbin/sysvinit || die
}

eselect-init_setup() {
	if eselect init >/dev/null 2>/dev/null; then
		"${ROOT}"/usr/bin/eselect init set systemd --use-old
	fi
}
...
pkg_postinst() {
	...
	elog "/sbin/init has been renamed to /sbin/sysvinit, in order to boot"
	elog "with the older sysvinit, you must add init=/sbin/sysvinit to"
	elog "the kernel boot command line."
}
