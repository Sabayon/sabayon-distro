...
src_install() {
	...
	if use hal ; then
		# This fixes HAL's 10-x11-input.fdi and 11-x11-synaptics.fdi
		mv "${S}/fdi/11-x11-synaptics.fdi" "${S}/fdi/09-x11-synaptics.fdi"

		insinto /usr/share/hal/fdi/policy/10osvendor
		doins "${S}"/fdi/09-x11-synaptics.fdi
...
pkg_postinst() {
	...
	if use hal ; then
		...
		elog "/etc/hal/fdi/policy/10osvendor/09-x11-synaptics.fdi"
		echo
