...
SRC_URI="ftp://ftp.trolltech.com/qt/source/qt-x11-${SRCTYPE}-${PV}.tar.bz2
	http://snigel.no-ip.com/~nxsty/linux/qt-x11-${SRCTYPE}-qt-copy-${QT_COPY_SNAP}-${PV}.patch.bz2
...
IUSE="cups debug doc examples firebird gif ipv6 mysql nas nis odbc opengl postgres sqlite xinerama immqt immqt-bc risky pertty"
...
src_unpack() {
	...
	# Apply the qt-copy stuff
	epatch "${DISTDIR}"/qt-x11-${SRCTYPE}-qt-copy-${QT_COPY_SNAP}-${PV}.patch.bz2
	sh apply_patches
	...
	use pertty && epatch ${FILESDIR}/qt-3.3.5-pertty.patch
